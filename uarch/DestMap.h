...
public:
        ...
        num_serdes(num_links), MAX_HMC(max_HMC_units)
    {
        ...
        m_selector_bits = myLog2(num_HMC);              // number of bits required to address HMC
        m_byte_offset_bits = myLog2(cache_line_size);   // number of LSBits that will have to be ignored while mapping address

        m_selector_mask = (0x1 << m_selector_bits) - 1; // once byte_offset_bits are ignored, use this mask to get HMC ID
    ...
    int lookup(uint64_t addr)
    {
        ...
        int idx = m_selector_mask & (adj_addr);         // HMC ID
        int m_id = 0;
        int p_rand = 0;
     ...
     * The local_addr which will be sent to 'that' HMC should not
     * contain the lower 2 bits + m_byte_offset_bits. Extension is similar to 5 - 8 HMCs.
     */
    uint64_t get_local_addr(uint64_t addr)
    {
    ...
    {
        uint64_t gaddr = (addr << m_selector_bits) | hmc_id;    // First insert the HMC id
        gaddr = gaddr << m_byte_offset_bits;                    // Append zeros to LS bits
#ifdef HMCDEBUG
