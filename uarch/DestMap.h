...
//    virtual int get_selector_bits(void) = 0;
//    virtual uint64_t get_selector_mask(void) = 0;
//    virtual int get_nodes_size(void) = 0;
};
...
public:
    ...
    uint64_t get_local_addr(uint64_t addr)
    {
		...
		std::cerr << "VaultMap addr: " << std::hex << addr << " laddr " << laddr
				<< " m_page_offset_bits " << m_page_offset_bits << " m_selector_bits " << m_selector_bits
...
public:
    	...
    	num_serdes(num_links), MAX_HMC(max_HMC_units)
    {
        ...
        m_selector_mask = (0x1 << m_selector_bits) - 1;
#ifdef HMCDEBUG
        std::cerr << "HMCMap m_selector_bits: " << m_selector_bits << " m_byte_offset_bits: "
